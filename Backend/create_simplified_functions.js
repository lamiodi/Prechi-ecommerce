...
async function createSimplifiedFunctions() {
  console.log('🔧 Creating simplified optimized functions...')
  ...
  try {
    ...
    // Create simplified get_product_or_bundle_optimized function
    console.log('🔄 Creating simplified get_product_or_bundle_optimized function...')
    await sql.unsafe(`
      ...
      BEGIN
        ...
        SELECT 
          ...
          COALESCE(
            ...
            GROUP BY pv.id, p2.name, c.color_name, bi.id
            ), '[]'::jsonb
          ) as variants,
          COALESCE(
            (SELECT jsonb_agg(image_url ORDER BY position) 
    ...
    console.log('✅ All simplified optimized functions have been created!')
...
createSimplifiedFunctions().catch(console.error)