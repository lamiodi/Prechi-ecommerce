...
interface CircularGalleryProps
  ...
  /**
   * Text color override for canvas titles (defaults to computed foreground)
   */
  textColor?: string;
  /**
...
function createTextTexture(
  ...
  // Extract font size safely
  let fontPx = 28;
  const match = font.match(/(\d+)px/);
  if (match) {
    fontPx = parseInt(match[1], 10);
  }
...
const CircularGallery = ({
  ...
  useEffect(() => {
    ...
    const computedColor = textColor || computedStyle.color || "hsl(var(--foreground))";
    const computedFontWeight = computedStyle.fontWeight || "bold";
    const computedFontSize = computedStyle.fontSize || "24px";
    const computedFontFamily = computedStyle.fontFamily || "Manrope, sans-serif";
  ...
  return (
    <div
      ...
      className={cn(
        ...
        "text-Primarycolor font-bold text-[24px] font-display",
        fontClassName,
        className,
      )}
      {...props}
