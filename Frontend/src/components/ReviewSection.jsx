      ...
      <div className="p-6 sm:p-8 lg:p-12">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8 gap-6">
          <div className="flex-1">
            <h2 className="text-2xl sm:text-3xl lg:text-4xl font-StoryScript font-bold text-Primarycolor mb-3">Customer Reviews</h2>
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
              <div className="flex items-center space-x-3">
                <StarRating rating={Math.round(averageRating)} />
                <span className="text-xl sm:text-2xl font-bold text-gray-900 font-StoryScript">{averageRating.toFixed(1)}</span>
              </div>
              <span className="text-gray-600 font-PatrickHand text-sm sm:text-base">Based on {totalReviews} reviews</span>
            </div>
          </div>
          <button
            onClick={() => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 mb-8">
          <div className="lg:col-span-1">
            <h3 className="text-lg font-semibold text-gray-900 mb-4 font-StoryScript">Rating Breakdown</h3>
          ...
          <div className="lg:col-span-2">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 w-full sm:w-auto">
                <div className="relative min-w-0 flex-1 sm:flex-none sm:w-40">
                  <select
                    ...
                    className="appearance-none bg-white border-2 border-gray-200 rounded-xl px-4 py-2.5 pr-8 text-sm font-medium hover:border-gray-300 focus:border-gray-900 focus:outline-none transition-colors w-full font-PatrickHand"
                  >
              ...
              <div className="text-sm text-gray-600 font-PatrickHand whitespace-nowrap">
        ...
        {error && (
          <div className="mb-8 p-4 bg-red-50 border-2 border-red-200 rounded-xl text-red-700 font-PatrickHand">
            {error}
          </div>
        )}
        {showWriteReview && isAuthenticated() && (
          <div className="mb-8 p-6 bg-gradient-to-br from-gray-50 to-white border-2 border-gray-200 rounded-2xl">
            <h3 className="text-xl font-semibold text-gray-900 mb-4 font-StoryScript">Write Your Review</h3>
            <form onSubmit={handleSubmitReview} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2 font-StoryScript">Rating</label>
                <StarRating
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2 font-StoryScript">Review Title</label>
                <input
                  type="text"
                  value={newReview.title}
                  onChange={(e) => setNewReview({ ...newReview, title: e.target.value })}
                  placeholder="Summarize your experience"
                  className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-gray-900 focus:outline-none transition-colors font-PatrickHand"
                  required
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2 font-StoryScript">Your Review</label>
                <textarea
                  value={newReview.comment}
                  onChange={(e) => setNewReview({ ...newReview, comment: e.target.value })}
                  placeholder={`Tell others about your experience with this ${bundleId ? 'bundle' : 'product'}`}
                  rows={4}
                  className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-gray-900 focus:outline-none transition-colors resize-none font-PatrickHand"
                  required
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2 font-StoryScript">Upload Images (Max 3)</label>
                <div
                  {...getRootProps()}
                ...
                >
                  <input {...getInputProps()} />
                  <p className="text-gray-600 font-PatrickHand">
                    {isDragActive
              ...
              <div className="flex flex-col sm:flex-row sm:space-x-4 space-y-4 sm:space-y-0">
                <button
                  type="submit"
                  disabled={uploading}
                  className={`w-full sm:w-auto px-6 py-3 bg-gray-900 text-white rounded-xl font-semibold hover:bg-gray-800 transition-all duration-200 font-PatrickHand ${uploading ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                ...
                <button
                  ...
                  className="w-full sm:w-auto px-6 py-3 border-2 border-gray-200 text-gray-700 rounded-xl font-semibold hover:border-gray-300 transition-all duration-200 font-PatrickHand"
                >
                  Cancel
                </button>
              </div>
        ...
        <div className="space-y-6">
          {filteredReviews.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-gray-600 font-PatrickHand text-lg">No reviews yet for this {bundleId ? 'bundle' : 'product'}.</p>
              <p className="text-gray-500 font-PatrickHand text-sm mt-2">Be the first to share your experience!</p>
            </div>
          ) : (
              ...
              <div key={review.id} className="p-6 sm:p-8 border-2 border-gray-100 rounded-2xl hover:border-gray-200 hover:shadow-lg transition-all duration-300 bg-white">
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-6">
                  <div className="flex items-start space-x-4 flex-1">
                    <div className="w-12 h-12 sm:w-14 sm:h-14 bg-gradient-to-br from-blue-100 to-blue-200 rounded-full flex items-center justify-center font-bold text-blue-700 font-StoryScript text-sm sm:text-base flex-shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-2">
                        <h4 className="font-semibold text-gray-900 font-StoryScript text-base sm:text-lg truncate">{review.user_name || 'Anonymous'}</h4>
                        <span className="text-sm text-gray-500 font-PatrickHand mt-1 sm:mt-0 sm:ml-4 flex-shrink-0">
                          {review.date ? new Date(review.date).toLocaleDateString() : 'Unknown Date'}
                        </span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <StarRating rating={review.rating} />
                        <span className="text-sm text-gray-600 font-PatrickHand">({review.rating}/5)</span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="mb-6">
                  <h5 className="font-semibold text-gray-900 mb-3 font-StoryScript text-lg">{review.title || 'No Title'}</h5>
                  <p className="text-gray-700 leading-relaxed font-PatrickHand text-base">{review.comment || 'No comment provided.'}</p>
                </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between pt-4 border-t border-gray-100 gap-4">
                  <div className="flex items-center space-x-1">
                    <span className="text-sm text-gray-600 font-PatrickHand mr-3">Was this helpful?</span>
                    <button
                      onClick={() => handleVote(review.id, 'helpful')}
                      className={`flex items-center space-x-1 px-3 py-1.5 rounded-full text-sm transition-all duration-200 font-PatrickHand ${
                  ...
                  <button className="text-sm font-PatrickHand text-gray-500 hover:text-gray-700 transition-colors self-start sm:self-center">
                    Report
                  </button>
                </div>
              </div>
        ...
        {filteredReviews.length > 0 && page * 10 < totalReviews && (
          <div className="text-center mt-10">
            <button
              ...
              className="w-full sm:w-auto px-8 py-4 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-xl font-semibold hover:from-blue-700 hover:to-blue-800 hover:shadow-lg transform hover:-translate-y-0.5 transition-all duration-200 font-PatrickHand"
            >
              Load More Reviews
            </button>
          </div>
